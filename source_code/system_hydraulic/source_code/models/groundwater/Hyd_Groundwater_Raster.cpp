...
//constructor
Hyd_Groundwater_Raster::Hyd_Groundwater_Raster(void) {
	...
	this->groundwater_pointer = NULL;
	this->geometrical_bound.set_elementtype_inside(_hyd_gw_elem_type::OTHER_GW_ELEM);
	this->index_gw_model = -1;

	//count the memory
	Sys_Memory_Count::self()->add_mem(sizeof(Hyd_Groundwater_Raster) - sizeof(Hyd_Groundwater_Polygon), _sys_system_modules::HYD_SYS);
}
//destructor
Hyd_Groundwater_Raster::~Hyd_Groundwater_Raster(void) {
	...
	//count the memory
	Sys_Memory_Count::self()->minus_mem(sizeof(Hyd_Groundwater_Raster) - sizeof(Hyd_Groundwater_Polygon), _sys_system_modules::HYD_SYS);
}
//_____________
//public
//set the geometric information
void Hyd_Groundwater_Raster::set_geometric_info(const _hyd_groundwater_geo_info geometric_info) {
	this->geo_info = geometric_info;
}
//Set the geometric information per database
void Hyd_Groundwater_Raster::set_geometric_info(QSqlTableModel *results, const int index) {
	this->geo_info.number_x = results->record(index).value((Hyd_Model_Groundwater::general_param_table->get_column_name(hyd_label::nx)).c_str()).toInt();
	this->geo_info.number_y = results->record(index).value((Hyd_Model_Groundwater::general_param_table->get_column_name(hyd_label::ny)).c_str()).toInt();
	this->geo_info.width_x = results->record(index).value((Hyd_Model_Groundwater::general_param_table->get_column_name(hyd_label::elemwidth_x)).c_str()).toDouble();
	this->geo_info.width_y = results->record(index).value((Hyd_Model_Groundwater::general_param_table->get_column_name(hyd_label::elemwidth_y)).c_str()).toDouble();
	this->geo_info.origin_global_x = results->record(index).value((Hyd_Model_Groundwater::general_param_table->get_column_name(hyd_label::lowleftx)).c_str()).toDouble();
	this->geo_info.origin_global_y = results->record(index).value((Hyd_Model_Groundwater::general_param_table->get_column_name(hyd_label::lowlefty)).c_str()).toDouble();
	this->geo_info.angle = results->record(index).value((Hyd_Model_Groundwater::general_param_table->get_column_name(hyd_label::angle)).c_str()).toDouble();

	this->make_boundary2polygon();

}
//Get the geometric information
_hyd_groundwater_geo_info Hyd_Groundwater_Raster::get_geometric_info(void) {
	return this->geo_info;
}
//Generate the model geometrical boundary as a polygon
void Hyd_Groundwater_Raster::make_boundary2polygon(void) {
	Geo_Point buffer[4];
	...
	try {
		...
		ostringstream info;
		info << "Try to set-up the geometrical groundwater raster boundary" << endl;
		msg.make_second_info(info.str());
		throw msg;
	}
}
//Set a geometrical raster polygon with a given element index without any pointer to the groundwater element
void Hyd_Groundwater_Raster::set_geometrical_raster_polygon(const int element_index) {
...
void Hyd_Groundwater_Raster::output_geometrical_boundary(void) {
	//output the elements
	ostringstream prefix1;
	prefix1 << "GEO_BOUND> ";
	Sys_Common_Output::output_hyd->set_userprefix(prefix1.str());
	ostringstream cout;
	cout << "Geometrical boundary of the groundwater model (global)" << endl;
...
void Hyd_Groundwater_Raster::output_geometrical_boundary2tecplot(ofstream *output_file) {
...
//output members
void Hyd_Groundwater_Raster::output_members(void) {
...
void Hyd_Groundwater_Raster::set_index_gw_model(const int index) {
	this->index_gw_model = index;
}
//Set a pointer to the groundwater elemnts
void Hyd_Groundwater_Raster::set_groundwater_elem_pointer(Hyd_Element_Groundwater *groundwater_pointer) {
	this->groundwater_pointer = groundwater_pointer;
};
//Generate and allocate the raster points
void Hyd_Groundwater_Raster::generate_alloc_points_segments(void) {
	this->delete_raster_points();
	...
	for (int j = 0; j < this->number_segments; j++) {
		...
		if (counter_y == counter_x) {
			...
			if (counter_x != 0 && j < this->number_segments - this->geo_info.number_x) {
				this->raster_segments[j].set_segmenttype(_hyd_groundwater_raster_segment_type::YFLOW_GW);
				if (this->groundwater_pointer != NULL) {
					this->raster_segments[j].set_element_ptr(&(this->groundwater_pointer[j - (counter_x*(this->geo_info.number_x + 1) + this->geo_info.number_x)]));
				}
			}
			//border
			else {
				this->raster_segments[j].set_segmenttype(_hyd_groundwater_raster_segment_type::BORDER_GW);
			}
			...
			//XFLOW
				...
				this->raster_segments[j].set_segmenttype(_hyd_groundwater_raster_segment_type::XFLOW_GW);
				if (this->groundwater_pointer != NULL) {
					this->raster_segments[j].set_element_ptr(&(this->groundwater_pointer[j - counter_x * (this->geo_info.number_x + 1)]));
				}
			}
...
void Hyd_Groundwater_Raster::delete_raster_segments(void) {
...
void Hyd_Groundwater_Raster::delete_raster_points(void) {
...
void Hyd_Groundwater_Raster::delete_points_segments(void) {
	this->delete_raster_points();
	this->delete_raster_segments();
}
//Get the pointer to all raster points
Hyd_Groundwaterraster_Point* Hyd_Groundwater_Raster::get_ptr_raster_points(void) {
	return this->raster_points;
}
//Get the pointer to all rastersegments
Hyd_Groundwaterraster_Segment* Hyd_Groundwater_Raster::get_ptr_raster_segments(void) {
	return this->raster_segments;
}
//Get the number of raster points
int Hyd_Groundwater_Raster::get_number_raster_points(void) {
	return this->number_points;
}
//Clone the raster points
void Hyd_Groundwater_Raster::clone_raster_points(Hyd_Groundwater_Raster *raster) {
	this->delete_raster_points();
...
Hyd_Groundwaterraster_Point * Hyd_Groundwater_Raster::get_raster_point(const int index) {
...
//Set a given type to Hyd_Element_Groundwater (e.g. noflow) if a raster polygon (Hyd_Groundwater_Raster_Polygon) is inside a given polygon. If the given polygon is a Hyd_Floodplain Polygon, the element type will only be changed if the given poly is a river polygon
//REVIEW currently river polygons are always designated as Hyd_Floodplain_Polygons
void Hyd_Groundwater_Raster::set_rasterpolygon_inside(Hyd_Floodplain_Polygon *polygon) {
...
void Hyd_Groundwater_Raster::set_rasterpolygon_inside(Hyd_Groundwater_Polygon *polygon) {
	for (int i = 0; i < this->number_polygons; i++) {
		Hyd_Multiple_Hydraulic_Systems::check_stop_thread_flag();
		//set the polygon
		this->set_raster_polygon(i);
...
void Hyd_Groundwater_Raster::set_rasterpolygon_intercepted(Hyd_Groundwater_Polygon *polygon, _hyd_gw_elem_type type) {
	for (int i = 0; i < this->number_polygons; i++) {
		//set the polygon
		Hyd_Multiple_Hydraulic_Systems::check_stop_thread_flag();
		this->set_raster_polygon(i);
...
//Set a given type to Hyd_Element_Groundwater (e.g. noflow) if a raster polygon (Hyd_Groundwater_Raster_Polygon) intercepts a given polygon 
void Hyd_Groundwater_Raster::set_rasterpolygon_intercepted(Hyd_Floodplain_Polygon *polygon, _hyd_gw_elem_type type) {
	for (int i = 0; i < this->number_polygons; i++) {
		//set the polygon
		Hyd_Multiple_Hydraulic_Systems::check_stop_thread_flag();
		this->set_raster_polygon(i);
...
void Hyd_Groundwater_Raster::interception_polysegments2raster(Hyd_Groundwater_Polysegment *line, Hyd_Param_Conductivity* conductivity_table) {
	try {
		for (int i = 0; i < this->number_polygons; i++) {
			//set the polygon
			this->set_raster_polygon(i);
		...
		if (line->get_line_type() == _hyd_groundwater_polysegment_type::GW_BARRIER) {
			//transfer the interception points to elements
			int elem_index = -1;
			Geo_Point buff1;
			Geo_Point buff2;
			for (int i = 1; i < line->get_number_of_points(); i++) {
				buff1 = (line->get_segment(i-1)->point1);
				buff2 = (line->get_segment(i - 1)->point2);
				double kf = 1e-5;
				int kf_ID = 0;
				kf_ID = line->get_segments_kf(i-1);
				kf = conductivity_table->get_con_value(kf_ID, _hyd_con_coefficient_types::Kf_coefficient);
				for (int j = 0; j < this->number_segments; j++) {
					//check the two points on line 
					if ((this->raster_segments[j].calc_point_on_segment(&buff1)) && (this->raster_segments[j].calc_point_on_segment(&buff2))) {
						if (this->raster_segments[j].get_segmenttype() == _hyd_groundwater_raster_segment_type::XFLOW_GW) {
							int index_x = j;
							int index_y	= index_x + this->geo_info.number_x;
							elem_index = this->segments_index2elem_index(index_x, index_y, _hyd_groundwater_polysegment_type::GW_BOUNDARYLINE);
							this->groundwater_pointer[elem_index].element_type->set_barrier_data(_hyd_all_element_direction_gw::X_DIRECT_GW, kf);
						}
						else if (this->raster_segments[j].get_segmenttype() == _hyd_groundwater_raster_segment_type::YFLOW_GW) {
							int index_y = j;
							int index_x = index_y - this->geo_info.number_x;
							elem_index = this->segments_index2elem_index(index_x, index_y, _hyd_groundwater_polysegment_type::GW_BOUNDARYLINE);
							this->groundwater_pointer[elem_index].element_type->set_barrier_data(_hyd_all_element_direction_gw::Y_DIRECT_GW, kf);
						}
					}
				...
				//this->groundwater_pointer[elem_index].element_type->set_barrier_data(direction,0);
			...
			//set the points to the Groundwater elements
...
void Hyd_Groundwater_Raster::interception_polysegments2raster(Hyd_Floodplain_Polysegment *line) {
	try {
		for (int i = 0; i < this->number_polygons; i++) {
			//set the polygon
			this->set_raster_polygon(i);
...
void Hyd_Groundwater_Raster::assign_elements2couplingpointlist(Hyd_Coupling_Point_RV2GW_List *list) {
	try {
		//first delete the interception list of the defining polysegment
		list->get_defining_polysegment_gw()->intercept_list.delete_list();
		//fill it with the interception points
		for (int i = 0; i < this->number_polygons; i++) {
			//set the polygon
			this->set_raster_polygon(i);
			//make the interception
			this->raster_elem.calculate_polysegment_interception(&(list->get_defining_polysegment_gw()->intercept_list), list->get_defining_polysegment_gw());
		}
		
		//sort the interception points in list after the distance to the beginning of the polysegment
		list->get_defining_polysegment_gw()->intercept_list.sort_distance();
		...
		int elem_index = -1;
		Hyd_Coupling_Point_RV2GW buffer;

		//count for the coupling points which coupled the same groundwater model
		int number = list->get_defining_polysegment_gw()->intercept_list.get_number_points();

		//just one point found (boundary element)
		if (list->get_defining_polysegment_gw()->intercept_list.get_number_points() == 1) {
			elem_index = this->convert_interception_points2elemindex(list->get_defining_polysegment_gw()->intercept_list.get_point(0));
			buffer.set_first_found_elem_index(elem_index);

			

			if (elem_index >= 0) {
				if (this->groundwater_pointer[elem_index].get_elem_type() == _hyd_gw_elem_type::RIVER_ELEM_GW) {
					this->search_convenient_coupling_element(&elem_index, *(list->get_defining_polysegment_gw()->get_first_point()), list->get_defining_polysegment_gw()->intercept_list.get_point(0).line_interception.interception_point, list->get_leftriver_bank_line());
				}
			}
			buffer.set_point((list->get_defining_polysegment_gw()->intercept_list.get_point(0).line_interception.interception_point));
			buffer.set_indices(list->get_defining_polysegment_gw()->intercept_list.get_point(0).index_intercepts, elem_index);
			buffer.set_groundwater_index(this->index_gw_model);
			if (elem_index >= 0) {
				buffer.set_pointer_groundwater_element(&this->groundwater_pointer[elem_index]);
			}
			else {
				buffer.set_pointer_groundwater_element(NULL);
			}
			buffer.set_point_name(hyd_label::coupling_point_RV2GW);
			list->set_new_couplingpoint(&buffer);
		}

		//more than one point
		for (int i = 1; i < list->get_defining_polysegment_gw()->intercept_list.get_number_points(); i++) {
			elem_index = this->convert_interception_points2elemindex(list->get_defining_polysegment_gw()->intercept_list.get_point(i - 1), list->get_defining_polysegment_gw()->intercept_list.get_point(i), list->get_defining_polysegment_gw()->get_line_type());
			buffer.set_first_found_elem_index(elem_index);

			//REVIEW
			//if (elem_index >= 0) {
			//	if (this->groundwater_pointer[elem_index].get_elem_type() == _hyd_gw_elem_type::RIVER_ELEM_GW || this->groundwater_pointer[elem_index].get_elem_type() == _hyd_gw_elem_type::LINE_ELEM_GW) {
			//		//|| this->groundwater_pointer[elem_index].get_elem_type()==_hyd_gw_elem_type::OTHER_GW_ELEM){
			//		this->search_convenient_coupling_element(&elem_index, list->get_defining_polysegment_gw()->intercept_list.get_point(i - 1).line_interception.interception_point, list->get_defining_polysegment_gw()->intercept_list.get_point(i).line_interception.interception_point, list->get_leftriver_bank_line());
			//	}
			//}
			...
			//if (i == 1) {
			//	buffer.set_point((list->get_defining_polysegment_gw()->intercept_list.get_point(0).line_interception.interception_point));
			//	buffer.set_gw_indices(elem_index);
			//	buffer.set_groundwater_index(this->index_gw_model);
			//	if (elem_index >= 0) {
			//		buffer.set_pointer_groundwater_element(&this->groundwater_pointer[elem_index]);
			//	}
			...
			buffer.set_point((list->get_defining_polysegment_gw()->intercept_list.get_point(i - 1).line_interception.interception_point));
			buffer.set_gw_indices(elem_index);
			//for (int j = 0; j < (list->get_defining_polysegment_gw()->get_number_segments()); j++) {
			//	//check if point is relevant=> it is inside the groundwater boundary
			//	if (list->get_defining_polysegment_gw()->my_segment[j].calc_point_on_segment(&(list->get_defining_polysegment_gw()->intercept_list.get_point(i - 1).line_interception.interception_point)) == true) {
			//		buffer.set_rv_indices(j);
			//		break;
			//	}
			//}
			buffer.set_groundwater_index(this->index_gw_model);
			...
			if (i == list->get_defining_polysegment_gw()->intercept_list.get_number_points() - 1) {
				buffer.set_point((list->get_defining_polysegment_gw()->intercept_list.get_point(i).line_interception.interception_point));
				buffer.set_gw_indices(elem_index);
				buffer.set_groundwater_index(this->index_gw_model);
				if (elem_index >= 0) {
					buffer.set_pointer_groundwater_element(&this->groundwater_pointer[elem_index]);
			...
			//check if the first profile and the last profile in the gw boundary
			if (this->find_elem_index_by_point(&(list->get_defining_polysegment_gw()->my_segment[0].point1)) != -1) {
				elem_index = this->find_elem_index_by_point(&(list->get_defining_polysegment_gw()->my_segment[0].point1));
				buffer.set_first_found_elem_index(elem_index);
				...
				buffer.set_point((list->get_defining_polysegment_gw()->my_segment[0].point1));
				buffer.set_gw_indices(elem_index);
				buffer.set_groundwater_index(this->index_gw_model);
				if (elem_index >= 0) {
					buffer.set_pointer_groundwater_element(&this->groundwater_pointer[elem_index]);
			...
			if (this->find_elem_index_by_point(&(list->get_defining_polysegment_gw()->my_segment[(list->get_defining_polysegment_gw()->get_number_segments())].point1)) != -1) {
				int num = list->get_defining_polysegment_gw()->get_number_segments();
				elem_index = this->find_elem_index_by_point(&(list->get_defining_polysegment_gw()->my_segment[num].point1));
				buffer.set_first_found_elem_index(elem_index);
				...
				buffer.set_point((list->get_defining_polysegment_gw()->my_segment[num].point1));
				buffer.set_gw_indices(elem_index);
				buffer.set_groundwater_index(this->index_gw_model);
				if (elem_index >= 0) {
					buffer.set_pointer_groundwater_element(&this->groundwater_pointer[elem_index]);
...
void Hyd_Groundwater_Raster::assign_elements2couplingpointlist(Hyd_Coupling_Point_GW2GW_List *list, const bool first_gw_flag) {
	try {
		//first delete the interception list of the defining polysegment
		list->get_defining_polysegment_gw()->intercept_list.delete_list();
		//for debug
		...
		//transfer the interception points to coupling points
		int elem_index = -1;
		Hyd_Coupling_Point_GW2GW buffer;
		Geo_Point buff1;
		Geo_Point buff2;
		for (int i = 1; i < list->get_defining_polysegment_gw()->intercept_list.get_number_points(); i++) {
			buff1 = (list->get_defining_polysegment_gw()->intercept_list.get_point(i - 1).line_interception.interception_point);
			buff2 = (list->get_defining_polysegment_gw()->intercept_list.get_point(i).line_interception.interception_point);
			elem_index = this->convert_interception_points2elemindex(list->get_defining_polysegment_gw()->intercept_list.get_point(i - 1), list->get_defining_polysegment_gw()->intercept_list.get_point(i), list->get_defining_polysegment_gw()->get_line_type());
			...
			if (i == 1) {
				...
				//set the index	
				if (first_gw_flag == true) {
					buffer.set_first_gwelem_index(elem_index);
					if (elem_index >= 0) {
						buffer.set_first_gwelem_pointer(&this->groundwater_pointer[elem_index]);
					}
					else {
						buffer.set_first_gwelem_pointer(NULL);
					}
					...
					_hyd_all_element_direction_gw direction;
					...
					buffer.set_second_gwelem_index(elem_index);
					if (elem_index >= 0) {
						buffer.set_second_gwelem_pointer(&this->groundwater_pointer[elem_index]);
					}
					else {
						buffer.set_second_gwelem_pointer(NULL);
					}
				}
				buffer.set_point_name(hyd_label::coupling_point_GW2GW);
				list->set_new_couplingpoint(&buffer);
			}
		...
		//if the groundwater model is inside the other one
		if (list->get_defining_polysegment_gw()->get_is_closed() == true) {
			buff1 = (list->get_defining_polysegment_gw()->intercept_list.get_last_point().line_interception.interception_point);
			buff2 = (list->get_defining_polysegment_gw()->intercept_list.get_first_point().line_interception.interception_point);
			elem_index = this->convert_interception_points2elemindex(list->get_defining_polysegment_gw()->intercept_list.get_last_point(), list->get_defining_polysegment_gw()->intercept_list.get_first_point(), list->get_defining_polysegment_gw()->get_line_type());
			//set the rest of points
			buffer.set_point(buff2);
			//set the index	
			if (first_gw_flag == true) {
...
int Hyd_Groundwater_Raster::find_elem_index_by_point(Geo_Point *point) {
	int index = -1;
...
int Hyd_Groundwater_Raster::find_elem_index_by_point_withboundary(Geo_Point *point) {
	int index = -1;
...
void Hyd_Groundwater_Raster::search_convenient_coupling_element(int *elem_index, Geo_Point point1, Geo_Point point2, const bool left_flag) {
	...
	elem_index_buff_1 = this->convert_2neighbouring_elemindex(_Hyd_Element_Groundwater_Type::get_next_element_direction_clockwise(direction), *elem_index);
	elem_index_buff_2 = this->convert_2neighbouring_elemindex(_Hyd_Element_Groundwater_Type::get_next_element_direction_counterclockwise(direction), *elem_index);
...
void Hyd_Groundwater_Raster::set_raster_segments2noflow(Hyd_Groundwater_Polygon *polygon) {

	for (int i = 0; i < this->number_segments; i++) {
		if (polygon->check_segment_is_inside(&this->raster_segments[i]) == true) {
			this->raster_segments[i].assign_value_flag2groundwater_element(true);
		}

	}
}
//Get the complete raster polygons
void Hyd_Groundwater_Raster::get_complete_raster_polygons(Hyd_Groundwaterraster_Polygon **polygons) {

	try {
		(*polygons) = new Hyd_Groundwaterraster_Polygon[this->number_polygons];
	}
	catch (bad_alloc &t) {
...
int Hyd_Groundwater_Raster::convert_2neighbouring_elemindex(_hyd_all_element_direction_gw direction, const int elem_index) {
	int new_elemindex = -1;
	...
	if (column == this->geo_info.number_x - 1 && (direction == _hyd_all_element_direction_gw::X_DIRECT_GW || direction == _hyd_all_element_direction_gw::XY_DIRECT_GW || direction == _hyd_all_element_direction_gw::MINUS_Y_X_DIRECT_GW)) {
		//there is no appropriate element
		return new_elemindex;
	}
	//element is in the last row
	if (row == this->geo_info.number_y - 1 && (direction == _hyd_all_element_direction_gw::Y_DIRECT_GW || direction == _hyd_all_element_direction_gw::XY_DIRECT_GW || direction == _hyd_all_element_direction_gw::YMINUS_X_DIRECT_GW)) {
		//there is no appropriate element
		return new_elemindex;
	}
	//element is in the leftest column
	if (column == 0 && (direction == _hyd_all_element_direction_gw::MINUS_X_DIRECT_GW || direction == _hyd_all_element_direction_gw::MINUS_X_MINUS_Y_DIRECT_GW || direction == _hyd_all_element_direction_gw::YMINUS_X_DIRECT_GW)) {
		//there is no appropriate element
		return new_elemindex;
	}
	//element is in the first row
	if (row == 0 && (direction == _hyd_all_element_direction_gw::MINUS_Y_DIRECT_GW || direction == _hyd_all_element_direction_gw::MINUS_X_MINUS_Y_DIRECT_GW || direction == _hyd_all_element_direction_gw::MINUS_Y_X_DIRECT_GW)) {
		//there is no appropriate element
		return new_elemindex;
	}

	//the element is not on a boundary
	if (direction == _hyd_all_element_direction_gw::X_DIRECT_GW) {
		new_elemindex = elem_index + 1;
	}
	else if (direction == _hyd_all_element_direction_gw::XY_DIRECT_GW) {
		new_elemindex = elem_index + 1 + this->geo_info.number_x;
	}
	else if (direction == _hyd_all_element_direction_gw::Y_DIRECT_GW) {
		new_elemindex = elem_index + this->geo_info.number_x;
	}
	else if (direction == _hyd_all_element_direction_gw::YMINUS_X_DIRECT_GW) {
		new_elemindex = elem_index + this->geo_info.number_x - 1;
	}
	else if (direction == _hyd_all_element_direction_gw::MINUS_X_DIRECT_GW) {
		new_elemindex = elem_index - 1;
	}
	else if (direction == _hyd_all_element_direction_gw::MINUS_X_MINUS_Y_DIRECT_GW) {
		new_elemindex = elem_index - 1 - this->geo_info.number_x;
	}
	else if (direction == _hyd_all_element_direction_gw::MINUS_Y_DIRECT_GW) {
		new_elemindex = elem_index - this->geo_info.number_x;
	}
	else if (direction == _hyd_all_element_direction_gw::MINUS_Y_X_DIRECT_GW) {
		new_elemindex = elem_index - this->geo_info.number_x + 1;
	}

	//check if the element type is convenient
	if (this->groundwater_pointer != NULL && new_elemindex >= 0) {
		if (this->groundwater_pointer[new_elemindex].get_elem_type() == _hyd_gw_elem_type::STANDARD_ELEM_GW || this->groundwater_pointer[new_elemindex].get_elem_type() == _hyd_gw_elem_type::LINE_ELEM_GW) {
			return new_elemindex;
		}
		else {
...
void Hyd_Groundwater_Raster::allocate_raster_points(void) {
	//allocate points
	try {
		this->raster_points = new Hyd_Groundwaterraster_Point[this->number_points];
	}
	catch (bad_alloc &t) {
...
void Hyd_Groundwater_Raster::allocate_raster_segments(void) {
	//allocate segments
	try {
		this->raster_segments = new Hyd_Groundwaterraster_Segment[this->number_segments];
	}
	catch (bad_alloc &t) {
		Error msg = this->set_error(0);
		ostringstream info;
...
void Hyd_Groundwater_Raster::set_raster_polygon(const int element_index) {
	int nx_counter = (int)(element_index / this->geo_info.number_x);
	//calculate the index
	int index_y = element_index + (2 * this->geo_info.number_x + 1)*(nx_counter + 1) - nx_counter * this->geo_info.number_x;
	int index_x = index_y - this->geo_info.number_x;
	...
	this->raster_elem.set_ptr2groundwater_element(&(this->groundwater_pointer[element_index]));
}
//Assign values (heights or noflow flags) and the corresponding index of a point to raster_point_list
void Hyd_Groundwater_Raster::assign_points_value2list(Hyd_Groundwater_Polysegment *line) {
	...
	for (int j = 0; j < line->intercept_list.get_number_points() - 1; j++) {
		...
		if (line->intercept_list.get_point(j).index_point_is_intercepted == -1 &&
			...
			//assign an _hyd_gw_elem_type to the intersected element
	...
	if (this->raster_point_list.get_number_points() > 1) {
		...
		if (line->intercept_list.get_last_point().index_point_is_intercepted != -1) {
			...
			buffer.flag = line->get_segments_no_flow_through(line->intercept_list.get_last_point().index_intercepts);
...
//Assign values (heights or noflow flags) and the corresponding index of a point to raster_point_list
void Hyd_Groundwater_Raster::assign_points_value2list(Hyd_Floodplain_Polysegment *line) {
	//for assigning the element type of intercepted elements
	int elem_index;
...
bool Hyd_Groundwater_Raster::assign_points_value2list_twopoints(_geo_multisegment_interception point_1, _geo_multisegment_interception point_2, Hyd_Groundwater_Polysegment *line) {
	//found flag
	...
	if (abs(point_1.index_point_is_intercepted - point_2.index_point_is_intercepted) == 1 ||
		...
		buffer.flag = line->get_segments_no_flow_through(index_line[0]);
		...
		buffer.flag = line->get_segments_no_flow_through(index_line[1]);
...
bool Hyd_Groundwater_Raster::assign_points_value2list_twopoints(_geo_multisegment_interception point_1, _geo_multisegment_interception point_2, Hyd_Floodplain_Polysegment *line) {
	//found flag
	bool found_flag = false;
	//index of the intercepting line segments (dikeline)
	int index_line[2];
...
bool Hyd_Groundwater_Raster::assign_points_value2list_segments(_geo_multisegment_interception point_1, _geo_multisegment_interception point_2, Hyd_Groundwater_Polysegment *line) {
	//found flag
	bool found_flag = false;
...
//Assign a value from a line to raster points (the two interception point are clear on the raster segments
bool Hyd_Groundwater_Raster::assign_points_value2list_segments(_geo_multisegment_interception point_1, _geo_multisegment_interception point_2, Hyd_Floodplain_Polysegment *line) {
	//found flag
	bool found_flag = false;
	//index of the intercepted raster segments
	int index_rast_seg[2];
...
bool Hyd_Groundwater_Raster::assign_points_value2list_point_segment(_geo_multisegment_interception point_1, _geo_multisegment_interception point_2, Hyd_Groundwater_Polysegment *line) {
...
bool Hyd_Groundwater_Raster::assign_points_value2list_point_segment(_geo_multisegment_interception point_1, _geo_multisegment_interception point_2, Hyd_Floodplain_Polysegment *line) {
...
bool Hyd_Groundwater_Raster::assign_points_value2list_segment_point(_geo_multisegment_interception point_1, _geo_multisegment_interception point_2, Hyd_Groundwater_Polysegment *line) {
	//found flag
	bool found_flag = false;
	...
	//set the nearest point to the hitted point of the second segment
		...
		buffer.flag = line->get_segments_no_flow_through(index_line);
...
bool Hyd_Groundwater_Raster::assign_points_value2list_segment_point(_geo_multisegment_interception point_1, _geo_multisegment_interception point_2, Hyd_Floodplain_Polysegment *line) {
	//found flag
	bool found_flag = false;
	//index of the intercepted raster segment (first point is just on segment)
	int index_rast_1 = point_1.index_is_intercept;
...
void Hyd_Groundwater_Raster::assign_listvalues2elements(Hyd_Groundwater_Polysegment *line) {
	...
	for (int i = 0; i < this->raster_point_list.get_number_points() - 1; i++) {
		...
		if (point1.index != point2.index) {
			...
			if (seg_index != -1 && seg_index < this->number_segments) {
				this->raster_segments[seg_index].assign_value_flag2groundwater_element(point1.flag);
			}
			else {
				//not needed
...
void Hyd_Groundwater_Raster::assign_listvalues2elements(Hyd_Floodplain_Polysegment *line) { //REVIEW This only changes the noflow_flag we do not need that for the river-groundwater coupling
	_hyd_value2pointindex point1;
	_hyd_value2pointindex point2;
	int seg_index = -1;
	//loop over all points in the list
...
int Hyd_Groundwater_Raster::convert_interception_points2elemindex(_geo_multisegment_interception point_1, _geo_multisegment_interception point_2, const _hyd_groundwater_polysegment_type type) {
	int elem_index = -1;
	//two interception points intercept the segments of the raster
...
int Hyd_Groundwater_Raster::convert_interception_points2elemindex(_geo_multisegment_interception point_1) {
	int elem_index = -1;
...
int Hyd_Groundwater_Raster::point_index2segment_index(const int point_index1, const int point_index2) {
	int seg_index = -1;
...
void Hyd_Groundwater_Raster::segment_index2point_index(const int seg_index, int *point_index1, int *point_index2) {
...
int Hyd_Groundwater_Raster::segments_index2elem_index(const int seg_index_1, const int seg_index_2, const _hyd_groundwater_polysegment_type type) {
	int elem_index = -1;
	//segment row
...
//Find element index, when two segments indices are given
int Hyd_Groundwater_Raster::segments_index2elem_index(const int seg_index_1, const int seg_index_2, const _hyd_floodplain_polysegment_type type) {
	int elem_index = -1;
	//segment row
	int row_1 = (int)(seg_index_1 / (this->geo_info.number_x * 2 + 1));
	int row_2 = (int)(seg_index_2 / (this->geo_info.number_x * 2 + 1));
...
int Hyd_Groundwater_Raster::points_index2elem_index(const int point_index_1, const int point_index_2) {
	int elem_index = -1;
	//point row
...
int Hyd_Groundwater_Raster::points_index2elem_index(const int point_index_1, const int point_index_2, const _hyd_groundwater_polysegment_type type) {
	int elem_index = -1;
	//point row
	int row_1 = (int)(point_index_1 / (this->geo_info.number_x + 1));
	int row_2 = (int)(point_index_2 / (this->geo_info.number_x + 1));
...
//Find element index, when two point indices are given; if the points are on the same segment the element index is decided with the linetype
int Hyd_Groundwater_Raster::points_index2elem_index(const int point_index_1, const int point_index_2, const _hyd_floodplain_polysegment_type type) {
	int elem_index = -1;
	//point row
	int row_1 = (int)(point_index_1 / (this->geo_info.number_x + 1));
	int row_2 = (int)(point_index_2 / (this->geo_info.number_x + 1));
...
int Hyd_Groundwater_Raster::points_segment_index2elem_index(const int seg_index, const int point_index, const _hyd_groundwater_polysegment_type type) {
	int elem_index = -1;
...
int Hyd_Groundwater_Raster::points_segment_index2elem_index(const int seg_index, const int point_index, const _hyd_floodplain_polysegment_type type) {
	int elem_index = -1;
...
void Hyd_Groundwater_Raster::set_elem_type_per_elemindex(const int elem_index, _hyd_gw_elem_type type) {
	if (elem_index >= 0 && elem_index < this->number_polygons) {
		this->groundwater_pointer[elem_index].set_element_type(type);
	}
}
void Hyd_Groundwater_Raster::set_elem_type_per_elemindex(const int elem_index, _hyd_elem_type type) {
	if (elem_index >= 0 && elem_index < this->number_polygons) {
		this->groundwater_pointer[elem_index].set_element_type(type);
	}
}
//Decide the element index by a given _hyd_groundwater_polysegment_type and two element indices REVIEW
int Hyd_Groundwater_Raster::decide_elemindex_per_type(const int index_1, const int index_2, const _hyd_groundwater_polysegment_type type) {
	int elem_index = -1;
	if (type == _hyd_groundwater_polysegment_type::RIVERBANKLINE_GW || type == _hyd_groundwater_polysegment_type::GW_BOUNDARYLINE) {
		if (index_1 == -1 && index_2 != -1) {
			if (this->groundwater_pointer[index_2].get_elem_type() == _hyd_gw_elem_type::STANDARD_ELEM_GW || this->groundwater_pointer[index_2].get_elem_type() == _hyd_gw_elem_type::LINE_ELEM_GW) {
				elem_index = index_2;
				return elem_index;
			}
		}
		else if (index_2 == -1 && index_1 != -1) {
			if (this->groundwater_pointer[index_1].get_elem_type() == _hyd_gw_elem_type::STANDARD_ELEM_GW || this->groundwater_pointer[index_1].get_elem_type() == _hyd_gw_elem_type::LINE_ELEM_GW) {
				elem_index = index_1;
				return elem_index;
			}
		}
			...
			else if (this->groundwater_pointer[index_2].get_elem_type() == _hyd_gw_elem_type::STANDARD_ELEM_GW || this->groundwater_pointer[index_2].get_elem_type() == _hyd_gw_elem_type::LINE_ELEM_GW) {
				elem_index = index_2;
				return elem_index;
			}
		}
...
int Hyd_Groundwater_Raster::decide_elemindex_per_type(const int index_1, const int index_2, const _hyd_floodplain_polysegment_type type) {
	int elem_index = -1;
	if (type == _hyd_floodplain_polysegment_type::RIVERBANKLINE || type == _hyd_floodplain_polysegment_type::FP_BOUNDARYLINE) {
		if (index_1 == -1 && index_2 != -1) {
			if (this->groundwater_pointer[index_2].get_elem_type() == _hyd_gw_elem_type::STANDARD_ELEM_GW || this->groundwater_pointer[index_2].get_elem_type() == _hyd_gw_elem_type::LINE_ELEM_GW) {
				elem_index = index_2;
				return elem_index;
...
_hyd_all_element_direction_gw Hyd_Groundwater_Raster::convert_angle2four_direction(const double angle) {
	_hyd_all_element_direction_gw direction;
	if (angle >= 0.0 && angle <= 45.0) {
		direction = _hyd_all_element_direction_gw::X_DIRECT_GW;
	}
	else if (angle > 45.0 && angle <= 135.0) {
		direction = _hyd_all_element_direction_gw::Y_DIRECT_GW;
	}
	else if (angle > 135.0 && angle <= 225.0) {
		direction = _hyd_all_element_direction_gw::MINUS_X_DIRECT_GW;
	}
	else if (angle > 225.0 && angle <= 315.0) {
		direction = _hyd_all_element_direction_gw::MINUS_Y_DIRECT_GW;
	}
...
_hyd_all_element_direction_gw Hyd_Groundwater_Raster::convert_angle2direction(const double angle) {
	_hyd_all_element_direction_gw direction;
	if (angle >= 0.0 && angle <= 22.5) {
		direction = _hyd_all_element_direction_gw::X_DIRECT_GW;
	}
	else if (angle > 22.5 && angle <= 67.5) {
		direction = _hyd_all_element_direction_gw::XY_DIRECT_GW;
	}
	else if (angle > 67.5 && angle <= 112.5) {
		direction = _hyd_all_element_direction_gw::Y_DIRECT_GW;
	}
	else if (angle > 112.5 && angle <= 157.5) {
		direction = _hyd_all_element_direction_gw::YMINUS_X_DIRECT_GW;
	}
	else if (angle > 157.5 && angle <= 202.5) {
		direction = _hyd_all_element_direction_gw::MINUS_X_DIRECT_GW;
	}
	else if (angle > 202.5 && angle <= 247.5) {
		direction = _hyd_all_element_direction_gw::MINUS_X_MINUS_Y_DIRECT_GW;
	}
	else if (angle > 247.5 && angle <= 292.5) {
		direction = _hyd_all_element_direction_gw::MINUS_Y_DIRECT_GW;
	}
	else if (angle > 292.5 && angle <= 337.5) {
		direction = _hyd_all_element_direction_gw::MINUS_Y_X_DIRECT_GW;
	}
...
_hyd_all_element_direction_gw Hyd_Groundwater_Raster::convert_angle2seconddirection(const double angle) {
	_hyd_all_element_direction_gw direction;
	if (angle >= 0.0 && angle <= 22.5) {
		direction = _hyd_all_element_direction_gw::XY_DIRECT_GW;
	}
...
double Hyd_Groundwater_Raster::calculate_distance_midpoints(const int index_1, const int index_2) {
	double distance = -1.0;

	if (index_1 < 0 || index_2 < 0 || index_1 >= this->number_polygons || index_2 >= this->number_polygons) {
		return distance;
	}
	distance = this->groundwater_pointer[index_1].get_mid_point()->distance(this->groundwater_pointer[index_2].get_mid_point());

	return distance;
}
//set the error
Error Hyd_Groundwater_Raster::set_error(const int err_type) {
	string place = "Hyd_Groundwater_Raster::";
	string help;
	string reason;
	int type = 0;
	bool fatal = false;
	...
	case 1://no segment index is found to the two points
		place.append("assign_listvalues2segments(Hyd_Groundwater_Polysegment *line)");
		reason = "Method is not used in the right way";
		help = "Check the code";
		type = 6;
		break;
	case 2://bad alloc
		place.append("get_complete_raster_polygons(Hyd_Groundwaterraster_Polygon **polygons)");
		reason = "Can not allocate the memory";
		help = "Check the memory";
		type = 10;
		break;
