...
//constructor
Hyd_Groundwaterraster_Polygon::Hyd_Groundwaterraster_Polygon(void) {
	this->ptr2groundwater_elem = NULL;
	this->segments = new Hyd_Groundwaterraster_Segment[4];
...
//___________
//public	
//Set the pointer to the groundwater element
void Hyd_Groundwaterraster_Polygon::set_ptr2groundwater_element(Hyd_Element_Groundwater *ptr2elem) {
	this->ptr2groundwater_elem = ptr2elem;
}
//Set the polygon's segments
void Hyd_Groundwaterraster_Polygon::set_polygon_segments(Hyd_Groundwaterraster_Segment *segment_y, Hyd_Groundwaterraster_Segment *segment_x, Hyd_Groundwaterraster_Segment *segment_minus_y, Hyd_Groundwaterraster_Segment *segment_minus_x) {
...
void Hyd_Groundwaterraster_Polygon::set_mid_point(Geo_Point *mid) {
	double x_buff;
	double y_buff;
...
void Hyd_Groundwaterraster_Polygon::is_inside_polygon(Hyd_Groundwater_Polygon *polygon) {
	...
	for (int i = 0; i < 4; i++) {
		if (polygon->check_point_inside(this->get_point(i)) == false) {
			//count how many points are not in the polygon
			count = count+1;
		}
	...
	if (inside == true) {
		this->ptr2groundwater_elem->set_element_type(polygon->get_elementtype_inside());
	}
	//}
}

//Check if this raster polygon is inside a River element and set element type to RIVER_ELEM
void Hyd_Groundwaterraster_Polygon::is_inside_polygon(Hyd_Floodplain_Polygon *polygon) {
	//    _polygon_intercept buff;
	//    buff=polygon->check_polygon_interception(this);
	//    if(buff==_polygon_intercept::inside_with_boundary || buff ==_polygon_intercept::complete_inside){
	//            this->ptr2floodplain_elem->set_element_type(polygon->get_elementtype_inside());
	...
	if (inside == true && polygon->get_elementtype_inside() == RIVER_ELEM_GW) {
		this->ptr2groundwater_elem->set_element_type(RIVER_ELEM_GW);
	}
	//}
}
//Check if this raster polygon is intercepted by the given Hyd_Groundwater_Polygon and set the given element type 
void Hyd_Groundwaterraster_Polygon::is_intercepted_polygon(Hyd_Groundwater_Polygon *polygon, _hyd_gw_elem_type type) {
	if (polygon->check_polygon_interception(this) == _polygon_intercept::partly_intercepted) {
		this->ptr2groundwater_elem->set_element_type(type);
	}
}
//Check if this raster polygon is intercepted by the given Hyd_Floodplain_Polygon and set the given element type 
void Hyd_Groundwaterraster_Polygon::is_intercepted_polygon(Hyd_Floodplain_Polygon *polygon, _hyd_gw_elem_type type) {
	if (polygon->check_polygon_interception(this) == _polygon_intercept::partly_intercepted) {
		this->ptr2groundwater_elem->set_element_type(type);
	}
}
//Calculate the interception points with a Geo_Poly_Segment and fill the Geo_Interception_Point_List 
void Hyd_Groundwaterraster_Polygon::calculate_polysegment_interception(Geo_Interception_Point_List *intercept_list, Geo_Polysegment *check_polysegment) {
...
//output members
void Hyd_Groundwaterraster_Polygon::output_members(ostringstream *cout) {
	for (int i = 0; i < this->number_segments; i++) {
		*cout << this->ptr2groundwater_elem->get_elem_number() << W(5) << i << W(12) << P(2) << FORMAT_FIXED_REAL << this->segments[i].point1->get_xcoordinate();
		*cout << W(12) << P(2) << FORMAT_FIXED_REAL << this->segments[i].point1->get_ycoordinate() << endl;
	}
}
//output the header (just once)
void Hyd_Groundwaterraster_Polygon::output_header(ostringstream *cout) {
	*cout << "No." << W(5) << "P.No" << W(8) << "x" << label::m << W(8) << "y" << label::m << endl;
}
//Get is-outside flag
bool Hyd_Groundwaterraster_Polygon::get_is_outside_flag(void) {
	return this->is_ouside;
}
//Set is-outside flag
void Hyd_Groundwaterraster_Polygon::set_is_outside_flag(const bool flag) {
	this->is_ouside = flag;
}
//_________
//private
//Sort the points
void Hyd_Groundwaterraster_Polygon::sort_points(void) {
