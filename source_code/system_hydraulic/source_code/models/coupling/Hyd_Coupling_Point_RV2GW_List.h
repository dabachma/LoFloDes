#pragma once
/**\class Hyd_Coupling_Point_RV2GW_List
	\author Bastian Winkels et al.
	\author produced by the Institute of Hydraulic Engineering (IWW), RWTH Aachen University
	\version 0.10.0
	\date 2022
*/
#ifndef HYD_COUPLING_POINT_RV2GW_LIST_H
#define HYD_COUPLING_POINT_RV2GW_LIST_H
...
//class Hyd_Coupling_RV2GW_Leakage;
class Hyd_Hydraulic_System;

///List-class where the coupling points between a river- (RV) and a groundwater-model (GW) are stored and managed\ingroup hyd 
/**
TODO change text
In this class the coupling points between a river- (RV) and a groundwater-model (GW) are stored and managed in a list.
The related polysegment to the list are the bank-lines (left/right in flow-direction) of the river-model (Hyd_Model_River).
This list is managed in Hyd_Coupling_RV2GW for one groundwater-model, alternatively in Hyd_Coupling_RV2GW_Merged
for multiple groundwater-models.
\see Hyd_Coupling_Point_RV2GW, Hyd_Model_Groundwater
*/
class Hyd_Coupling_Point_RV2GW_List : public _Hyd_Coupling_Point_List_GW
{

public:
	///Default constructor
	Hyd_Coupling_Point_RV2GW_List(void);
	///Default destructor
	~Hyd_Coupling_Point_RV2GW_List(void);
	...
	Hyd_Coupling_Point_RV2GW* get_total_list(void);
	///Get a specific coupling points with the given index
	Hyd_Coupling_Point_RV2GW get_couplingpoint(const int index);
	///Get a pointer to specific coupling points with the given index
	Hyd_Coupling_Point_RV2GW* get_ptr_couplingpoint(const int index);

	///Get the next coupling point upwards in the list
	Hyd_Coupling_Point_RV2GW* get_next_point_upwards(Hyd_Coupling_Point_RV2GW* coupling_point);
	///Get the next coupling point downwards in the list
	Hyd_Coupling_Point_RV2GW* get_next_point_downwards(Hyd_Coupling_Point_RV2GW* coupling_point);

	///Get the first coupling points of the list
	Hyd_Coupling_Point_RV2GW get_first_couplingpoint(void);
	///Get the last coupling points of the list
	Hyd_Coupling_Point_RV2GW get_last_couplingpoint(void);

	///Output the setted members
	void output_setted_members(ostringstream *cout);

	///Set a new coupling points
	void set_new_couplingpoint(Hyd_Coupling_Point_RV2GW *new_point);
	...
	void set_midtriver_line(const bool flag);
	//Get if it is the mid river line
	bool get_midriver_line(void);

	///Find a coupling point in the list in which area the given distance from the beginning of the river lies in
	Hyd_Coupling_Point_RV2GW* find_coupling_point(const double distance, _Hyd_Coupling_Point *point);
	///Find a corresponding coupling point to a given distance along the defining polysegment
	Hyd_Coupling_Point_RV2GW* find_coupling_point(const double distance);
	...
	void assign_point2polysegment(void);
	...
	void add_relevant_polysegment_points(Hyd_Groundwater_Polygon *gw_boundary);
	...
	void merge_coupling_list(Hyd_Coupling_Point_RV2GW_List *merging_list);
	...
	void clone_list(Hyd_Coupling_Point_RV2GW_List *list, Hyd_Hydraulic_System *system, const int rv_number);

	///Get pointer to the coupling point specified by the given
	Hyd_Coupling_Point_RV2GW* get_ptr_couplingpoint(Hyd_Coupling_Point_RV2GW coupling_point);


	//Output the coupling points to csv_file
	void output_result2csv_1d(void);
	/*
